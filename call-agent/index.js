...
### Role
You are an AI assistant named Sophie, working at Opticlaim. Your role is to assist customers with insurance claim settlements, answer questions about policies, guide them through the claims process and register new claims.
### Persona
    ...
    fastify.get('/media-stream', { websocket: true }, (connection, req) => {
        ...
        openAiWs.on('message', async (data) => {
            try {
                ...
                // Handle function calls (for Q&A and filing a claim)
                if (response.type === 'response.function_call_arguments.done') {
                    ...
                    } else if (functionName === 'file_claim') {  // If the file_claim function is called
                        const policyNumber = args.policy_number;  // Get the policy number
                        const incidentDetails = args.incident_details;  // Get the incident details
                        try {
                            const webhookResponse = await sendToWebhook({
                                route: "4",  // Route 4 for filing a claim
                                data1: policyNumber,
                                data2: incidentDetails  // Send the incident details to the webhook
                            });
                            ...
                            // Parse the webhook response
                            const parsedResponse = JSON.parse(webhookResponse);
                            const claimMessage = parsedResponse.message || "I'm sorry, I couldn't file the claim at this time.";
                            ...
                            const functionOutputEvent = {
                                ...
                                item: {
                                    type: "function_call_output",
                                    role: "system",
                                    output: claimMessage,  // Provide the claim status
                                }
                            };
                            openAiWs.send(JSON.stringify(functionOutputEvent));  // Send the claim status back to OpenAI

                            // Trigger AI to generate a response based on the claim
                            openAiWs.send(JSON.stringify({
                                type: "response.create",
                                response: {
                                    modalities: ["text", "audio"],
                                    instructions: `Inform the user about the claim status: ${claimMessage}. Be concise and friendly.`,
                                }
                            }));
                        } catch (error) {
                            console.error('Error filing claim:', error);
                            sendErrorResponse();  // Send an error response if filing fails
                        }
                    }
                }
